...
ShaguPlates.uf = CreateFrame("Frame", nil, UIParent)
ShaguPlates.uf:SetScript("OnUpdate", function()
  if InCombatLockdown and not InCombatLockdown() then
    for frame in pairs(ShaguPlates.uf.delayed) do
      frame:UpdateVisibility()
      ShaguPlates.uf.delayed[frame] = nil
    end
  end
end)

ShaguPlates.uf.frames = {}
ShaguPlates.uf.delayed = {}

local glow = {
  edgeFile = ShaguPlates.media["img:glow"], edgeSize = 8,
...
local aggrodata = { }
function ShaguPlates.api.UnitHasAggro(unit)
...
ShaguPlates.uf.glow = CreateFrame("Frame")
ShaguPlates.uf.glow:SetScript("OnUpdate", function()
...
ShaguPlates.uf.glow.mod = 0
ShaguPlates.uf.glow.val = 0

function ShaguPlates.uf.glow.UpdateGlowAnimation()
  this:SetAlpha(ShaguPlates.uf.glow.val)
end

function ShaguPlates.uf:UpdateVisibility()
  local self = self or this

  -- we're infight, delay the update
  if InCombatLockdown and InCombatLockdown() then
    ShaguPlates.uf.delayed[self] = true
    return
  end
  ...
  -- display every unit as player while ShaguPlates.uf.showall is set
  if ShaguPlates.uf.showall then
    self._label = self._label or self.label
    self._id = self._id or self.id
    self.label, self.id = "player", ""
  elseif not ShaguPlates.uf.showall and self._label and self._id then
...
function ShaguPlates.uf:UpdateFrameSize()
...
function ShaguPlates.uf:UpdateConfig()
  local f = self
  ...
  f.glow:SetScript("OnUpdate", ShaguPlates.uf.glow.UpdateGlowAnimation)
  f.glow:Hide()
  ...
  ShaguPlates.api.CreateBackdrop(f.hp, default_border)

  f.hp.bar:SetStatusBarTexture(ShaguPlates.media[f.config.bartexture])
  ...
  if custombg == "1" then
    local cr, cg, cb, ca = ShaguPlates.api.strsplit(",", custombgcolor)
  ...
  ShaguPlates.api.CreateBackdrop(f.power, default_border)
  f.power.bar:SetStatusBarTexture(ShaguPlates.media[f.config.bartexture])
  ...
  if custompbg == "1" then
    local cr, cg, cb, ca = ShaguPlates.api.strsplit(",", custompbgcolor)
    cr, cg, cb, ca = tonumber(cr), tonumber(cg), tonumber(cb), tonumber(ca)
  ...
  elseif f.config.portrait == "left" then
    ...
    ShaguPlates.api.CreateBackdrop(f.portrait, default_border)
  ...
  if f.config.hitindicator == "1" then
    f.feedbackText:SetFont(ShaguPlates.media[f.config.hitindicatorfont], f.config.hitindicatorsize, "OUTLINE")
  ...
  f.hpLeftText:SetFont(ShaguPlates.font_unit, C.global.font_unit_size, "OUTLINE")
  ...
  f.hpRightText:SetFont(ShaguPlates.font_unit, C.global.font_unit_size, "OUTLINE")
  ...
  f.hpCenterText:SetFont(ShaguPlates.font_unit, C.global.font_unit_size, "OUTLINE")
  ...
  f.powerLeftText:SetFont(ShaguPlates.font_unit, C.global.font_unit_size, "OUTLINE")
  ...
  f.powerRightText:SetFont(ShaguPlates.font_unit, C.global.font_unit_size, "OUTLINE")
  ...
  f.powerCenterText:SetFont(ShaguPlates.font_unit, C.global.font_unit_size, "OUTLINE")
  ...
  f.incHeal.texture:SetTexture(ShaguPlates.media["img:bar"])
  f.incHeal.texture:SetVertexColor(0, 1, 0, 0.5)
  f.incHeal:Hide()

  f.incHeal:SetScript("OnShow", function()
    if ShaguPlates.prediction and f.label and f.id then
      ShaguPlates.prediction:TriggerUpdate(UnitName(f.label .. f.id))
    end
  end)
  ...
  f.ressIcon.texture:SetTexture(ShaguPlates.media["img:ress"])
  ...
  f.pvpIcon.texture:SetTexture(ShaguPlates.media["img:pvp"])
  ...
  f.raidIcon.texture:SetTexture(ShaguPlates.media["img:raidicons"])
  ...
  f.happinessIcon.texture:SetTexture(ShaguPlates.media["img:neutral"])
  ...
  if f.config.buffs == "off" then
    ...
    for i=1, 32 do
      ...
      f.buffs[i] = f.buffs[i] or CreateFrame("Button", "ShaguPlates" .. f.fname .. "Buff" .. i, f)
      ...
      f.buffs[i].stacks:SetFont(ShaguPlates.font_unit, C.global.font_unit_size, "OUTLINE")
      ...
      elseif f:GetName() == "pfTarget" and ShaguPlates.expansion == "tbc" then
  ...
  if f.config.debuffs == "off" then
    ...
    for i=1, 32 do
      ...
      f.debuffs[i] = f.debuffs[i] or CreateFrame("Button", "ShaguPlates" .. f.fname .. "Debuff" .. i, f)
      ...
      f.debuffs[i].stacks:SetFont(ShaguPlates.font_unit, C.global.font_unit_size, "OUTLINE")
  ...
  if f.config.visible == "1" then
    ShaguPlates.uf:RefreshUnit(f, "all")
    f:EnableScripts()
    f:EnableEvents()
    f:UpdateFrameSize()
  else
...
function ShaguPlates.uf.OnShow()
  ShaguPlates.uf:RefreshUnit(this)
end

function ShaguPlates.uf.OnEvent()
  -- update indicators
  ...
  then
    ShaguPlates.uf:RefreshIndicators(this)
  end
  ...
  if event == "PLAYER_ENTERING_WORLD" then
    ShaguPlates.uf:RefreshUnit(this, "all")
  ...
  elseif this.label == "player" and (event == "PLAYER_AURAS_CHANGED" or event == "UNIT_INVENTORY_CHANGED") then
    ShaguPlates.uf:RefreshUnit(this, "aura")
  ...
  elseif arg1 and arg1 == this.label .. this.id then
    if event == "UNIT_PORTRAIT_UPDATE" or event == "UNIT_MODEL_CHANGED" then
      ShaguPlates.uf:RefreshUnit(this, "portrait")
    elseif event == "UNIT_AURA" then
      ShaguPlates.uf:RefreshUnit(this, "aura")
    elseif event == "UNIT_FACTION" then
      ShaguPlates.uf:RefreshUnit(this, "pvp")
...
function ShaguPlates.uf.OnUpdate()
  ...
  if this.lastTick and this.lastTick < GetTime() then
    ...
    ShaguPlates.uf:RefreshUnitState(this)
    ShaguPlates.uf:RefreshIndicators(this)

    if this.config.glowaggro == "1" and ShaguPlates.api.UnitHasAggro(this.label .. this.id) > 0 then
...
function ShaguPlates.uf.OnEnter()
  if not this.label then return end
...
function ShaguPlates.uf.OnLeave()
  GameTooltip:FadeOut()
end

function ShaguPlates.uf.OnClick()
  if not this.label and this.unitname then
    TargetByName(this.unitname, true)
  else
    ShaguPlates.uf:ClickAction(arg1)
  end
end

function ShaguPlates.uf:RightClickAction(unit)
  if unit == "player" then
...
function ShaguPlates.uf:EnableEvents()
  local f = self
...
function ShaguPlates.uf:EnableScripts()
  local f = self

  -- handle secure unit button templates (> vanilla)
  if ShaguPlates.client > 11200 then
    f.showmenu = ShaguPlates.uf.RightClickAction
    f:SetAttribute("*type1", "target")
    f:SetAttribute("*type2", "showmenu")
  else
    f:SetScript("OnClick", ShaguPlates.uf.OnClick)
  end

  f:SetScript("OnShow", ShaguPlates.uf.OnShow)
  f:SetScript("OnEvent", ShaguPlates.uf.OnEvent)
  f:SetScript("OnUpdate", ShaguPlates.uf.OnUpdate)
  f:SetScript("OnEnter", ShaguPlates.uf.OnEnter)
  f:SetScript("OnLeave", ShaguPlates.uf.OnLeave)
  f:EnableClickCast()
...
function ShaguPlates.uf:CreateUnitFrame(unit, id, config, tick)
  ...
  f.UpdateFrameSize  = ShaguPlates.uf.UpdateFrameSize
  f.UpdateVisibility = ShaguPlates.uf.UpdateVisibility
  f.UpdateConfig     = ShaguPlates.uf.UpdateConfig
  f.EnableScripts    = ShaguPlates.uf.EnableScripts
  f.EnableEvents     = ShaguPlates.uf.EnableEvents
  f.EnableClickCast  = ShaguPlates.uf.EnableClickCast
  f.GetColor         = ShaguPlates.uf.GetColor

  -- cache values to the frame
  f.label = unit
  f.fname = fname
  f.id = id
  f.config = config or ShaguPlates_config.unitframes.fallback
  f.tick = tick
  ...
  f.visibility = f.visibility or CreateFrame("Frame", nil, ShaguPlates)
  ...
  table.insert(ShaguPlates.uf.frames, f)
  return f
end

function ShaguPlates.uf:RefreshUnitState(unit)
  local alpha = unit.alpha_visible
  local unlock = ShaguPlates.unlock and ShaguPlates.unlock:IsShown() or nil
  ...
  elseif unit.config.faderange == "1" and not ShaguPlates.api.UnitInRange(unit.label .. unit.id, 4) and not unlock then
  ...
  if unit.config.portrait == "bar" then
    unit.portrait:SetAlpha(ShaguPlates_config.unitframes.portraitalpha)
  end
...
function ShaguPlates.uf:RefreshIndicators(unit)
  ...
  if unit.happinessIcon and unit:GetName() == "pfPet" then -- Happiness Icon
    if unit.config.happinessicon == "0" then
      ...
      if UnitIsVisible("pet") then
        ...
        if happiness == 1 then
          unit.happinessIcon.texture:SetTexture(ShaguPlates.media["img:sad"])
          unit.happinessIcon.texture:SetVertexColor(1, 0, 0, 1)
        elseif happiness == 2 then
          unit.happinessIcon.texture:SetTexture(ShaguPlates.media["img:neutral"])
          unit.happinessIcon.texture:SetVertexColor(1, 1, 0, 1)
        else
          unit.happinessIcon.texture:SetTexture(ShaguPlates.media["img:happy"])
...
function ShaguPlates.uf:RefreshUnit(unit, component)
  ...
  if component == "all" or ( component == "all" or component == "aura" ) then
    if not unit.dispellable and unit.config.debuff_indicator ~= "0" then
      unit.dispellable = ShaguPlates.uf:SetupDebuffFilter((unit.config.debuff_ind_class == "0" and true or nil))
    ...
    if table.getn(unit.dispellable) > 0 then
      ...
      for _, debuff in pairs(unit.dispellable) do
        ...
        if indicator.size ~= indicator[debuff].size or disptype ~= indicator[debuff].disp then
          if disptype == "4" then
            indicator[debuff].tex:SetTexture(ShaguPlates.media["img:"..debuff])
    ...
    if not unit.indicators and unit.config.buff_indicator == "1" then
      unit.indicators = ShaguPlates.uf:SetupBuffIndicators(unit.config)
        ...
        if texture then
          ...
          for _, filter in pairs(unit.indicators) do
            if filter == string.lower(texture) then
              ShaguPlates.uf:AddIcon(unit, pos, texture, timeleft, count)
              pos = pos + 1
              break
            end
          end
    ...
    for pos=pos, 6 do ShaguPlates.uf:HideIcon(unit, pos) end
  end
  ...
  if customfullhp == "1" and UnitHealth(unitstr) == UnitHealthMax(unitstr) then
    r, g, b, a = ShaguPlates.api.strsplit(",", customcolor)
    custom_active = true
  ...
  if utype == 0 then
    r, g, b, a = ShaguPlates.api.strsplit(",", mana)
  elseif utype == 1 then
    r, g, b, a = ShaguPlates.api.strsplit(",", rage)
  elseif utype == 2 then
    r, g, b, a = ShaguPlates.api.strsplit(",", focus)
  elseif utype == 3 then
    r, g, b, a = ShaguPlates.api.strsplit(",", energy)
  end
  unit.power.bar:SetStatusBarColor(r, g, b, a)

  if UnitName(unitstr) then
    unit.hpLeftText:SetText(ShaguPlates.uf:GetStatusValue(unit, "hpleft"))
    unit.hpCenterText:SetText(ShaguPlates.uf:GetStatusValue(unit, "hpcenter"))
    unit.hpRightText:SetText(ShaguPlates.uf:GetStatusValue(unit, "hpright"))

    unit.powerLeftText:SetText(ShaguPlates.uf:GetStatusValue(unit, "powerleft"))
    unit.powerCenterText:SetText(ShaguPlates.uf:GetStatusValue(unit, "powercenter"))
    unit.powerRightText:SetText(ShaguPlates.uf:GetStatusValue(unit, "powerright"))
  ...
  ShaguPlates.uf:RefreshUnitState(unit)
end
...
function ShaguPlates.uf:EnableClickCast()
  ...
  for bid, button in pairs(buttons) do
    for modifier, mconf in pairs(modifiers) do
      ...
      if ShaguPlates_config.unitframes["clickcast"..bconf..mconf] ~= "" then
        ...
        if ShaguPlates.client > 11200 then
          ...
          if string.find(ShaguPlates_config.unitframes["clickcast"..bconf..mconf], "^%/(.+)") then
            self:SetAttribute(prefix.."type"..bid, "macro")
            self:SetAttribute(prefix.."macrotext"..bid, ShaguPlates_config.unitframes["clickcast"..bconf..mconf])
            self:SetAttribute(prefix.."spell"..bid, nil)
          else
            self:SetAttribute(prefix.."type"..bid, "spell")
            self:SetAttribute(prefix.."spell"..bid, ShaguPlates_config.unitframes["clickcast"..bconf..mconf])
          ...
          self.clickactions[modifier..button] = ShaguPlates_config.unitframes["clickcast"..bconf..mconf]
        end
      end
    end
  end
...
function ShaguPlates.uf:ClickAction(button)
  ...
  if button == "RightButton" then
    ShaguPlates.uf:RightClickAction(label)
  else
...
function ShaguPlates.uf:AddIcon(frame, pos, icon, timeleft, stacks)
  ...
  for i=1,6 do
    if not frame.icon[i] then
      ...
      frame.icon[i].stacks:SetFont(ShaguPlates.font_unit, math.max(iconsize/3, 10), "OUTLINE")
...
function ShaguPlates.uf:HideIcon(frame, pos)
...
function ShaguPlates.uf:SetupDebuffFilter(allclasses)
...
function ShaguPlates.uf:SetupBuffIndicators(config)
  local _, myclass = UnitClass("player")
  local indicators = {}
...
function ShaguPlates.uf:GetLevelString(unitstr)
...
function ShaguPlates.uf:GetStatusValue(unit, pos)
  ...
  if ShaguPlates.libhealth and ShaguPlates.libhealth.enabled then
    rhp, rhpmax = ShaguPlates.libhealth:GetUnitHealth(unitstr)
  ...
  if config == "unit" then
    ...
    local level = unit:GetColor("level") .. ShaguPlates.uf:GetLevelString(unitstr)
    return level .. "  " .. name
  ...
  elseif config == "level" then
    return unit:GetColor("level") .. ShaguPlates.uf:GetLevelString(unitstr)
  elseif config == "class" then
    return unit:GetColor("class") .. (UnitClass(unitstr) or UNKNOWN)

  -- health
  elseif config == "health" then
    return unit:GetColor("health") .. ShaguPlates.api.Abbreviate(rhp)
  elseif config == "healthmax" then
    return unit:GetColor("health") .. ShaguPlates.api.Abbreviate(rhpmax)
  ...
  elseif config == "healthmiss" then
    ...
    elseif health == 0 then
      ...
      return unit:GetColor("health") .. ShaguPlates.api.Abbreviate(health)
    end
  elseif config == "healthdyn" then
    if hp ~= hpmax then
      return unit:GetColor("health") .. ShaguPlates.api.Abbreviate(rhp) .. " - " .. ceil(hp / hpmax * 100) .. "%"
    else
      return unit:GetColor("health") .. ShaguPlates.api.Abbreviate(rhp)
    end
  ...
  elseif config == "healthminmax" then
    return unit:GetColor("health") .. ShaguPlates.api.Abbreviate(rhp) .. "/" .. ShaguPlates.api.Abbreviate(rhpmax)

  -- mana/power/focus
  elseif config == "power" then
    return unit:GetColor("power") .. ShaguPlates.api.Abbreviate(mp)
  elseif config == "powermax" then
    return unit:GetColor("power") .. ShaguPlates.api.Abbreviate(mpmax)
  ...
  elseif config == "powermiss" then
    ...
    if power == 0 then
      ...
      return unit:GetColor("power") .. ShaguPlates.api.Abbreviate(power)
    end
  elseif config == "powerdyn" then
    if mp ~= mpmax then
      return unit:GetColor("power") .. ShaguPlates.api.Abbreviate(mp) .. " - " .. ceil(mp / mpmax * 100) .. "%"
    else
      return unit:GetColor("power") .. ShaguPlates.api.Abbreviate(mp)
    end
  elseif config == "powerminmax" then
    return unit:GetColor("power") .. ShaguPlates.api.Abbreviate(mp) .. "/" .. ShaguPlates.api.Abbreviate(mpmax)
  else
    return ""
  end
end

function ShaguPlates.uf.GetColor(self, preset)
